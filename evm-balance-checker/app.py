...
def get_defi_position(address: str, token_address: str, token_name: str, protocol: str, chain: str) -> Optional[Dict]:
    """Get DeFi protocol position (aTokens, cTokens, LP tokens, staked tokens, etc.)"""
    try:
        w3 = web3_instances.get(chain)
        if not w3:
            return None
        ...
        print(f"DeFi Position: {protocol_name} - {token_name} -> {underlying} = ${balance_usd:.2f}")
